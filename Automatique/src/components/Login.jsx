// src/components/CreativeLogin.jsx
...
gsap.registerPlugin(); // ScrollTrigger non utilisé dans la conversion mais garde la possibilité
...
export default function Login() {
    ...
    useEffect(() => {
        const ctx = gsap.context(() => {
            // --- query elements inside the componen
            // t root ---
            setTimeout(() => {
                const ctxRoot = rootRef.current;
                const containerEl = containerRef.current;
                const checkboxEl = ctxRoot.querySelector('.form-container .form-row input[type="checkbox"]');
                const nameEl = ctxRoot.querySelector('.form-container .form-row input[name="name"]');
                const passwordEl = ctxRoot.querySelector('.form-container .form-row input[name="password"]');
                const emailEl = ctxRoot.querySelector('.form-container .form-row input[name="email"]');
                const submitBtn = ctxRoot.querySelector('.form-container .form-row input[type="submit"]');

                const sprayer = ctxRoot.querySelector('.sprayer');
                const sprayHandContainer = ctxRoot.querySelector('.spray-hand-container');
                const sprayLines = Array.from(ctxRoot.querySelectorAll('.spray-line'));
                const sprayBubbles = Array.from(ctxRoot.querySelectorAll('.spray-bubble'));

                const pushingHand = ctxRoot.querySelector('.pushing-hand');
                const sprayerHead = ctxRoot.querySelector('.sprayer-head');
                const gearsContainer = ctxRoot.querySelector('svg .gears');
                const gearConnector = ctxRoot.querySelector('.gear-connector');

                const pullSystemContainer = ctxRoot.querySelector('.pull-system');

                const checkboxPullLine = ctxRoot.querySelector('.checkbox-pull-line');
                const checkboxPullCircle = ctxRoot.querySelector('.checkbox-pull-circle');
                const btnPullLine = ctxRoot.querySelector('.submit-btn-connector');
                const btnHandlerCircle = ctxRoot.querySelector('.submit-btn-circle');

                const spiralContainer = ctxRoot.querySelector('.spiral-container');
                const weightBigContainer = ctxRoot.querySelector('.weight-big-container');

                const scalesContainer = ctxRoot.querySelector('.scales-container');
                const scalesLine = ctxRoot.querySelector('.scales-moving-line');
                const weightBig = ctxRoot.querySelector('.weight-big');
                const spiralPath = ctxRoot.querySelector('.spiral-path');

                const carContainer = ctxRoot.querySelector('.car-container');
                const car = ctxRoot.querySelector('.car');
                const carInclineWrapper = ctxRoot.querySelector('.car-container g');
                const timingChains = Array.from(ctxRoot.querySelectorAll('.timing-chain'));
                const reelsConnector = ctxRoot.querySelector('.reels-connector');
                const carWeightConnector = ctxRoot.querySelector('.car-weight-connector');

                const grabbingHand = ctxRoot.querySelectorAll('.grabbing-hand');
                const grabbingHandOpenFingers = Array.from(ctxRoot.querySelectorAll('.grabbing-hand-finger-open'));
                const grabbingHandClosedFingers = Array.from(ctxRoot.querySelectorAll('.grabbing-hand-finger-closed'));

                // --- pasted & adapted script.js logic ---
                function scaleToFit() {
                    const h = 800;
                ...
                nameEl.addEventListener('input', () => {
                    ...
                    if (nameValid) {
                        ...
                        gsap.to(submitBtn, { duration: .3, color: "rgba(255, 255, 255, 20)" });
                    }
                });
                ...
                // --- helper functions (copied & slightly adapted) ---
                function layoutPreparation() {
                    ...
                    if (timingChains[0]) {
                        gsap.set(timingChains[0], { attr: { "strokeWidth": "5", "strokeDasharray": "0 12" } });
                    }
                    if (timingChains[1]) {
                        gsap.set(timingChains[1], { attr: { "strokeWidth": "5", "strokeDasharray": "0 12" } });
                    }
                    gsap.set(checkboxPullLine, { attr: { y1: -105, y2: 44 } });
                    gsap.set(submitBtn, { transformOrigin: "90% 0%", rotation: -90 });
                ...
                function createEmailTl() {
                    ...
                    const tl = gsap.timeline({
                        ...
                        .fromTo(timingChains[0], { attr: { "strokeDashoffset": 2 } }, { duration: .15, attr: { "strokeDashoffset": 20 } }, hammerTimeStart)
                        .fromTo(timingChains[1], { attr: { "strokeDashoffset": 24 } }, { duration: .15, attr: { "strokeDashoffset": 6 } }, hammerTimeStart)
                        ...
                        if (dIdx === (data.length - 1)) {
                            tl.eventCallback("onUpdate", () => {
                                ...
                                if (deltaY > 8) {
                                    ...
                                    gsap.set(sprayLines, { attr: { "strokeDashoffset": 70 * sprayProgress }, opacity: Math.pow(bubblesOpacity, 2) });
                                ...
                                gsap.set(gearConnector, { attr: { x1: gear.center.x + handleRadius * Math.cos(angle), y1: gear.center.y + handleRadius * Math.sin(angle), x2: 700 + 18, y2: 646 - 100 + deltaY } });
                            });
                ...
                return () => {
                    ...
                    // optionally kill GSAP contexts/timelines if needed
                };
    ...
    return (
        <div>
            <div className="w-[40%] ml-2 relative h-screen overflow-hidden">
                <video
                    className="absolute inset-0 object-cover w-full h-full clip-video"
            ...
            <div className="w-[50%] ">
                <div className="absolute top-0 right-0 w-[60%] h-16 flex items-center justify-end pr-5">
                    <h1 className="text-white font-bold text-lg px-6 py-2 
    bg-[#0a0a0a] rounded-[30px] shadow-[0_0_20px_rgba(0,0,0,0.5)]
    relative overflow-hidden liquid-button">
                        Creer votre compte
                    </h1>
                </div>


                <div ref={rootRef} className="container mt-20 bg-blue-75">

                    <div className="relative flex flex-col form-container ">
                        <div className="w-[5%] absolute top-[-45%] right-[100%]  mr-11">
                            <AnimatedTitle
                                title="    L<b>o</b>gin "
                                containerClass="!text-black text-start"
                            />
                        </div>
                        ...
                        <label className="form-row">
                            <input
                                ...
                                className="w-full h-12 px-4 text-black placeholder-black transition-all duration-300 border shadow-md  bg-white/10 backdrop-blur-xl border-white/20 rounded-xl shadow-black/20 focus:border-blue-400 focus:ring-2 focus:ring-blue-500/40"
                            />

                        </label>

                        <label className="relative form-row mt-11">
                            <input
                                ...
                                required
                                autoComplete="one-time-code"
                                className="w-full h-12 px-4 text-black placeholder-black transition-all duration-300 border shadow-md  bg-white/10 backdrop-blur-lg border-white/20 rounded-xl shadow-black/20 focus:border-black focus:ring-2 focus:black"
                            />
                            <button
                                type="button"
                                onClick={() => setShowPassword(!showPassword)}
                                className="absolute w-4 h-4 text-black -translate-y-1/2 top-1/2 right-3"
                            >
                        ...
                        <label className="form-row">
                            <input
                                type="checkbox"
                                name="subscribe"
                                className="w-5 h-5 transition-all border rounded-md  border-white/30 bg-white/10 checked:bg-blue-600"
                            />

                            Enregistrer votre compte
                        </label>

                        <div className="form-row  ml-[-35px]">
                            <input
                                type="submit"
                                value="Envoyer"
                                className="
                    ...
                    <svg ref={svgRef} viewBox="0 0 1000 1000" className="pointer-events-none w-[1000px]" strokeLinecap="round" strokeLinejoin="round">
                        ...
                        <g className="weight-big-container">
                            {/* <line x1="11" x2="60" y1="14" y2="14"></line>
